...
export function rowsToCSV(cols, rows) {
...
export function rowsToTableHTML(cols, rows) {
    const head = cols.map((c) => `<th>${htmlEscape(c.label)}</th>`).join("");
    const body = rows
        .map((r) => `<tr>${cols.map((c) => `<td>${htmlEscape(String(r[c.key] ?? ""))}</td>`).join("")}</tr>`)
        .join("");
...
export function printTable(title, tableHTML) {
    const w = window.open("", "_blank");
    if (!w)
        return;
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${htmlEscape(title)}</title><style>body{font-family:Inter,system-ui,sans-serif;font-size:12px;padding:24px;color:#101112}h1{font-size:16px;margin:0 0 12px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #e2e0e8;padding:6px 8px;text-align:left}th{background:#f4f4f6}</style></head><body><h1>${htmlEscape(title)}</h1>${tableHTML}</body></html>`);
    w.document.close();
    w.focus();
    w.print();
}
